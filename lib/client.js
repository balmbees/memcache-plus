...
Client.prototype.connectToHosts = function() {
    ...
    const connectedCallback = (newConnection) => {
        connectedCount ++;
        if (connectedCount === this.hosts.length) {
            this.flushBuffer();
        }
    }
    ...
    this.hosts.forEach((host) => {
        ...
        this.connections[host] = new Connection({
            ...
            onConnect: function(/** this: Connection */) {
...
Client.prototype.ready = function() {
    if (this.autodiscover && !this.ring) {
        return false;
    }
...
Client.prototype.getConnection = function(options) {
    return this.connections[this.getConnectionName(options)];
}

Client.prototype.getConnectionName = function (options) {
    const { command, args } = options;
    if (command === "get_multi") {
        // Among multiple keys, use "first one" as hashkey
        return this.ring.get(args[0][0]);
    } else {
        return this.ring.get(args[0]);
    }
}
