...
Connection.prototype.read = function(data) {
    ...
    } else if (deferred.type === 'get_multi') {
        if (data.match(/^VALUE .+/)) {
            var spl = data.match(/^VALUE ([^ ]+) ([0-9]+) ([0-9]+)( ([0-9]+))?$/);
            deferred.values.push({
                key: spl[1],
                flag: Number(spl[2]),
                len: Number(spl[3]),
            });
            ...
            resp = deferred.values;

            done = true;
        } else {
            _.last(deferred.values).value = data;
            done = false;
        }
...
/**
 * get_multi() - Get a value on this connection
 *
 * @param {Array<String>} keys - The key for the value to retrieve
 * @param {Object} [opts] - Any additional options for this get
 * @returns {Promise}
 */
Connection.prototype.get_multi = function (keys, opts) {
    opts = opts || {};

    // Do the get
    const deferred = misc.defer(keys);
    deferred.type = "get_multi";
    deferred.values = [];

    this.commandBuffer = this.commandBuffer.push(deferred);

    this.write(`get ${keys.join(" ")}`);
    ...
    return deferred.promise
        ...
        .then(function (values) {
            ...
            values.forEach(({ key, flag, len, value }) => {
                results[key] = convert(value, flag, len);
            });

            // For "Missing" Keys, Fill with explicit undefined
            keys.forEach((key) => {
